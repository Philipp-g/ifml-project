...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    old_features, old_step = state_to_features(old_game_state)
    new_features, new_step = state_to_features(new_game_state)
    ...
    #self.transitions.append(Transition(state_to_features(old_game_state),self_action,state_to_features(new_game_state),rewards,0))
    self.total_steps += 1
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    for event in events:
        self.event_logger[event] += 1
    # Take last transition + permutations and mark as done
    last_transistions = []
    rewards = reward_from_events(self, events)
    self.reward.append(rewards)
    for i in range(8):
        last_trans = self.transitions.pop()
        last_trans = Transition(last_trans.state, last_trans.action, last_trans.next_state, last_trans.reward + rewards, 1)
        last_transistions.append(last_trans)
    self.transitions.extend(last_transistions)
...
def sample_from_transitions(self, batch_size: int) -> Union[
    ...
    # probs = np.full(batch_size, 0.9)
    # probs_r = np.random.random_sample(batch_size)
    # cond = list(probs_r < probs)
    # batch = [x if c else y
    #          for c, x, y in
    #          zip(cond, random.sample(self.transitions, batch_size), random.sample(self.other_transitions, batch_size))]
...
def reward_from_events(self, events: List[str]) -> float:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        e.LOOPED: -0.05,
        e.WAITED: -0.05,
        e.CLOSER_TO_TARGET: 0.02,
        e.FARTHER_FROM_TARGET: -0.02,
        e.INVALID_ACTION: -0.2,  # punish harder
        e.KILLED_SELF: -0.5,
        e.CRATE_DESTROYED: 0.1,
    }
    # Dont punish loop event if good event occurs at same time
