...
def state_to_features(self, game_state: dict) -> Optional[Tuple[torch.Tensor, torch.Tensor]]:
    """
    *This is not a required function, but an idea to structure your code.*
    ...
    if game_state["step"] == 1:
        self.others[game_state["self"][0]] = {}
        random.shuffle(game_state["others"])
        for i, (name, p, b, (x, y)) in enumerate(game_state["others"]):
            self.others[game_state["self"][0]][name] = i

    field_channel[game_state["self"][3]] = 3

    for name, p, b, (x, y) in game_state["others"]:
        field_channel[(x, y)] = self.others[game_state["self"][0]][name] + 4
